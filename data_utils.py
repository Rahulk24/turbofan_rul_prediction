"""
Data loading and preprocessing utilities for the NASA C-MAPSS
...
# ---------------------------------------------------------------------------
# Column names as defined in the C-MAPSS readme file
# ---------------------------------------------------------------------------
...
# Sensors that are (near) constant in FD001/FD003 (single operating condition)
# and therefore carry no degradation information. Determined empirically from
# the training data (std ~ 0) and confirmed against published C-MAPSS studies.
...
def load_dataset(data_dir, subset):
    """Load train, test and RUL files for a given subset, e.g. 'FD001'."""
...
def add_train_rul(train):
    """Compute the (piecewise-linear-capped) RUL target for every row of the
    training set. Standard approach: RUL = min(max_cycle - cycle, RUL_CAP).
    """
...
def get_test_last_cycle(test):
    """Return the last recorded cycle for every test unit -- this is the
    snapshot the competition asks us to estimate the RUL for."""
    return test.sort_values(["unit", "cycle"]).groupby("unit").tail(1).reset_index(drop=True)


def select_features(subset):
    """Feature columns to use for a given subset (drop flat sensors for the
    single-condition subsets FD001/FD003; keep everything for the six-condition
    subsets FD002/FD004 since sensors that look flat overall are actually
    condition-dependent there)."""
...
def phm_score(y_true, y_pred, a1=10, a2=13):
    """Official PHM08 asymmetric scoring function (Saxena et al. 2008, eq. 11).
    Penalizes late predictions (over-estimating RUL) more heavily than early
    ones."""
